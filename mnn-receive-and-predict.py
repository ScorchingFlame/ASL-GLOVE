...
async def validate_and_predict(data: list):
    ...
    predictions = {labels:0 for labels in models_with_labels.values()}
    for model in models.items():
        ...
        indexofprediction = 0
        for o in prediction:
            predictions[models_with_labels[model][indexofprediction]] += o
            indexofprediction += 1
    print(predictions, max(prediction, key= lambda x: prediction[x]))
